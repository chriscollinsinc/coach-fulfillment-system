...
function resyncPreview2026(){
  ...
  for(const cid in perClient){ const pc=perClient[cid];
    ...
    const tryMatch=(v)=>{ let bi=-1; for(let i=0;i<cad.length;i++){if(claimed[i]||cad[i].completed)continue;bi=i;break;} if(bi>=0)return bi; let bd=1e15; for(let i=0;i<cad.length;i++){if(claimed[i])continue;const d=dd(v.week,cad[i].due);if(d<bd){bd=d;bi=i;}} return bi; };
