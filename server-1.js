...
route('POST', /^\/api\/place-past-visit$/, ['admin','lead','sales','coach'], (req, res, m, body, user) => {
  ...
  db.prepare('UPDATE visits SET cal_coach=?, cal_week=? WHERE id=?')
    .run(body.coach_id, body.week, body.visit_id);
  
  log(user.email, 'visit.place_past', {visit_id: body.visit_id, coach_id: body.coach_id, week: body.week});
  send(res, 200, {ok: true});
});
...
route('GET', /^\/api\/clients\/(\d+)\/notes-by-cycle$/, ['admin','lead','sales','coach'], (req, res, m) => {
  try {
    ...
    const notes = db.prepare(`
      SELECT
        v.id, COALESCE(v.completed_date, v.due, date('now')) as note_date, 'Visit Note' as note_type,
        ...
        COALESCE(v.completed_date, v.due, date('now')) as created, v.cycle, v.program, v.contract_id,
